...
class PuzzleGenerator implements PuzzleSource {
    ...
    static Sq findUniqueSuccessor(Model model, Sq start) {
        ...
        for (int i = 0; i < start.successors().size(); i += 1) {
            int x = start.successors().get(i).x;
            int y = start.successors().get(i).y;
            if ((model.get(x, y).sequenceNum() != 0)
                    && (start.sequenceNum() != 0)) {
                if (start.connectable(model.get(x, y))) {
                    return model.get(x, y);
                }
            } else {
                if (start.connectable(model.get(x, y))) {
                    trueCount += 1;
                    answer = model.get(x, y);
                }
            }
        }
        if (trueCount == 1) {

...
public class MatrixUtils {
    ...
    public static double[][] accumulateVertical(double[][] m) {
        ...
        for (int row = 1; row < m.length; row += 1) {
            for (int col = 0; col < m[0].length; col += 1) {
               if (col == 0) {
                   am[row][col] = Math.min(am[row-1][col], am[row-1][col+1]) + m[row][col];
               }
               else if (col == m[0].length -1 ) {
                   am[row][col] = Math.min(am[row-1][col], am[row-1][col-1]) + m[row][col];
               }
               else {
                   am[row][col] = min(am[row-1][col-1], am[row-1][col], am[row-1][col+1]) + m[row][col];
               }
            }
        }
        return am;
    ...
    public static double[][] accumulate(double[][] m, Orientation orientation) {
        if (orientation == Orientation.VERTICAL) {
            ...
            return Trans(accumulateVertical(Trans(m)));
        }
    }
...
//     *  1000000     73403     35399   1000000
//     *  1000000   1000000   1000000   1000000
        double[][] matrix = new double[][] {{1000000,1000000,1000000,1000000},
                {1000000,75990,30003,1000000},
                {1000000,30002,103046,1000000},
                {1000000,29515,38273,1000000},
                {1000000,73403,35399,1000000},
        ...
        System.out.println(matrixToString(accumulateVertical(matrix)));
    }

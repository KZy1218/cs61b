...
public class CompoundInterest {
    ...
    static double futureValueReal(double presentValue, double rate,
        ...
        return futureValue(presentValue, rate, targetYear) * Math.pow(1 - 0.01 * inflationRate, numYears(targetYear));
    }
    ...
    static double totalSavings(double perYear, int targetYear, double rate) {
        int t = numYears(targetYear);
        double tS = 0;
        while (t > 0){
            tS += futureValue(perYear, rate, targetYear + 1 - t);
            t -= 1;
        }
        return tS + perYear;
    }
